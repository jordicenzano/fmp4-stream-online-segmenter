...
describe('fmp4-chunklist-generator', function() {
    ...
    let input_vod_file_name = path.join(moduleDir,'fixtures/source_15s_fmp4.mp4');
    let base_path = moduleDir; //Test dir
    let out_path = path.join(base_path, 'results');

    const input_vod_file_name_size = fs.statSync(input_vod_file_name).size;
    ...
    describe('generate a live event chunklist and chunks from file', function () {
        ...
        it('ffmpeg generated fmp4 - read by big chunks', function (done) {
            ...
            let chunklist_filename = path.join(out_path, path.parse(input_vod_file_name).name + '_event.mpd');
        ...
        it('ffmpeg generated fmp4 - read byte by byte', function (done) {
            ...
            let readFileStream = new fs.createReadStream(input_vod_file_name, { highWaterMark: 1 });
