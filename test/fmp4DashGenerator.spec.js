...
const enChunklistType = require('../src/dashManifest.js').enChunklistType;
...
describe('fmp4-chunklist-generator', function() {
    ...
    describe('generate a vod event manifest using range requests from video file and SegmentList', function () {
        ...
        it('ffmpeg generated fmp4 - read by big chunks and 4s chunk target dur', function (done) {
            let chunklist_filename = path.join(out_path, path.parse(input_v_vod_file_name).name + '_event_block.mpd');

            let segmenter = new underTest.fmp4DashGenerator(false, "./", path.basename(input_v_vod_file_name), 4, enChunklistType.SegmentList);
    ...
    <AdaptationSet>
      <Representation id="video01" mimeType="video/mp4" codecs="avc1.42C00D" bandwidth="24596">
        ...
        it('ffmpeg generated fmp4 - read byte by byte and 4s chunk target dur', function (done) {
            let chunklist_filename = path.join(out_path, path.parse(input_v_vod_file_name).name + '_event_byte.mpd');
